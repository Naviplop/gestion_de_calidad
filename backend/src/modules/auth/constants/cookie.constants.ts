...
export const CSRF_COOKIE_NAME = 'x-csrftoken';
export const CSRF_COOKIE_PATH = '/';
export const CSRF_COOKIE_MAX_AGE = 7 * 24 * 60 * 60 * 1000;
...
export function buildCsrfCookieOptions(options: AuthCookieOptions = {}): Required<AuthCookieOptions> {
  const isProduction = process.env.NODE_ENV === 'production';
  return {
    httpOnly: options.httpOnly ?? false,
    secure: options.secure ?? isProduction,
    sameSite: options.sameSite ?? (isProduction ? 'strict' : 'lax'),
    path: options.path ?? CSRF_COOKIE_PATH,
